...
userSchema.methods.getProjectStats = async function () {
  const [agg] = await mongoose.model("Project").aggregate([
    { $match: { owner: this._id } },
    {
      $group: {
        _id: null,
        projectCount: { $sum: 1 },
        sourceCount: { $sum: { $size: { $ifNull: ["$sources", []] } } },
        completedProjects: {
  ...
  return {
    ...
    totalSources: agg?.sourceCount || 0,
